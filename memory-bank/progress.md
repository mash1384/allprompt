...
### 단계 21: 디자인 컨셉 및 스타일 가이드 정의 (완료)
- 미니멀리즘, 명확성, 일관성, 전문성을 주요 디자인 원칙으로 설정하고 다크 모드 기반 스타일 가이드 정의:
  - **색상 팔레트 정의:** 매우 어두운 회색 배경(#20232A, #282C34), 패널 구분 배경색(#1A1D23, #32363F), 밝은 회색/흰색 텍스트(#ABB2BF, #DCE0E8), 강조/선택 색상(#528BFF, #61AFEF), 경고/오류 색상(#E5C07B, #E06C75)
  - **폰트 선택:** 가독성 높은 시스템 기본 산세리프 폰트 사용, 텍스트 계층 구조에 따른 크기 및 굵기 차별화
  - **아이콘 스타일:** 단색의 미니멀한 라인 아이콘 스타일 채택, SVG 형식 활용
  - **기본 레이아웃 및 간격 규칙:** 컴포넌트 간 일관된 여백 적용으로 시각적 리듬감 제공
- 스타일시트 구현:
  - QSS를 활용한 종합적 스타일시트 작성 (src/gui/resources/styles/style.qss)
  - 모든 UI 요소에 일관된 디자인 적용: 버튼, 메뉴, 체크박스, 트리뷰, 스크롤바, 패널 등
  - 트리 뷰, 체크박스 등 특수 UI 요소의 아이콘 교체를 위한 SVG 파일 생성 (branch_closed.svg, branch_open.svg, check.svg)
- Qt 리소스 시스템 통합:
  - resources.qrc 파일을 통한 스타일시트 및 아이콘 리소스 관리
  - 리소스 컴파일 스크립트 구현 (compile_resources.py)
  - main.py 파일 수정하여 애플리케이션 시작 시 스타일시트 자동 로드
- MainWindow 클래스 UI 요소 개선:
  - 패널, 버튼, 레이블에 objectName 지정하여 스타일시트 선택자와 연결
  - 패널 구분을 위한 헤더 레이블 추가
  - UI 요소 배치 미세 조정으로 가독성 및 시각적 계층 구조 향상
- 구현 혜택:
  - 직관적이고 현대적인 다크 모드 인터페이스로 장시간 사용 시 눈의 피로도 감소
  - 모든 UI 요소에 일관된 디자인 언어 적용으로 전문적인 느낌 강화
  - 시각적 계층 구조 명확화로 사용성 향상
  - Qt 리소스 시스템을 통해 외부 의존성 없이 자체 포함된 디자인 리소스 관리

### 단계 22: 메인 레이아웃 및 패널 재구성 (완료)
- `src/gui/main_window.py`의 UI 레이아웃 구성 개선:
  - 명확한 2단 레이아웃 구조 강화:
    - 왼쪽 패널은 파일 트리 뷰, 오른쪽 패널은 제어 및 정보 영역
    - 각 패널에 시각적 구분을 위한 테두리와 둥근 모서리 적용
    - 정확한 여백(마진, 패딩) 설정으로 UI 요소 간 균형 개선
  - 기능 섹션별 명확한 구분과 헤더 추가:
    - "폴더 선택", "선택 정보", "작업" 섹션을 명확히 구분
    - 각 섹션마다 시각적으로 구분되는 헤더 레이블 추가
    - 일관된 레이아웃 컨테이너와 간격 사용
  - 정보 표시 개선:
    - 현재 폴더 경로를 별도 레이블로 분리하여 가독성 향상
    - 선택 정보를 그룹화하여 관련 데이터 시각적 결합
    - "클립보드에 복사" 버튼 강조 및 크기 확대
- `src/gui/resources/styles/style.qss` 스타일시트 개선:
  - 패널 디자인 개선:
    - 패널 테두리 및 모서리 둥글기 적용으로 시각적 분리감 강화
    - 헤더 레이블에 하단 경계선과 색상 차별화 적용
  - 버튼 및 상호작용 요소 개선:
    - 버튼 크기 및 패딩 증가로 클릭 영역 확대
    - 주요 액션 버튼("클립보드에 복사") 시각적 강조
    - 체크박스 및 트리뷰 항목의 상호작용 시각 효과 개선
  - 텍스트 요소 최적화:
    - 헤더와 내용 텍스트의 크기 및 색상 차별화
    - 현재 경로 표시 레이블에 배경색 및 테두리 적용
    - 카운터 레이블(토큰 수) 강조를 위한 굵은 글꼴 및 크기 증가
  - 기타 시각적 개선:
    - 툴팁 패딩 및 글꼴 크기 증가로 가독성 향상
    - 트리 뷰에 테두리 및 패딩 적용으로 콘텐츠 구분감 향상
    - 스크롤바 디자인과 애니메이션 효과 개선
- 스타일 변경사항 적용을 위한 리소스 컴파일:
  - `compile_resources.py` 스크립트 실행으로 변경된 스타일시트 반영
  - 모든 UI 요소에 적절한 `objectName` 설정으로 스타일 선택자 연결
- 구현 혜택:
  - 직관적이고 명확한 UI 구조로 사용자 경험 향상
  - 시각적 계층 구조 명확화로 주요 기능 인지성 개선
  - 일관된 디자인 언어 적용으로 전문적인 느낌 강화
  - 요소 간 적절한 여백과 구분으로 가독성 향상

### 단계 23: 파일 트리 뷰 스타일 개선 (미니멀 아이콘, 명확한 들여쓰기, 깔끔한 상태 표시) (완료)
- 직관적이고 미니멀한 디자인의 SVG 아이콘 세트 추가:
  - `folder.svg`: 폴더 아이콘
  - `file.svg`: 기본 파일 아이콘
  - `code_file.svg`: 코드 파일 아이콘
  - `doc_file.svg`: 문서 파일 아이콘
  - `symlink.svg`: 심볼릭 링크 아이콘
  - `binary_file.svg`: 바이너리 파일 아이콘
  - `error.svg`: 에러 상태 아이콘
  - `image_file.svg`: 이미지 파일 아이콘
- `resources.qrc` 파일 업데이트: 새 아이콘 리소스 등록
- `style.qss` 파일 개선:
  - 트리 뷰 항목 높이 및 패딩 최적화 (26px 높이)
  - 항목 간 여백 추가 및 둥근 모서리 적용
  - 호버 및 선택 상태의 시각적 효과 개선
  - 트리 아이템 체크박스 스타일 커스터마이징
  - 일관된 색상 및 폰트 적용
  - 트리 헤더 스타일 정의
- `main_window.py`의 `_init_icons` 메서드 개선:
  - 기존 시스템 아이콘에서 커스텀 SVG 아이콘으로 전환
  - 파일 확장자별 아이콘 매핑 유지
- 구현 혜택:
  - 파일 유형별 직관적인 시각적 구분
  - 더 미니멀하고 세련된 디자인
  - 트리 뷰 요소들의 일관된 스타일
  - 선택 및 호버 상태의 명확한 시각적 표현
  - 전체 UI와 조화를 이루는 아이콘 세트
- 테스트 내용:
  - 다양한 파일 유형과 구조를 가진 폴더 로딩
  - 체크박스 선택/해제 및 부모/자식 아이템 상태 변화 확인
  - 심볼릭 링크 및 바이너리 파일의 시각적 표현 확인
  - 호버/선택 상태의 스타일 검증
